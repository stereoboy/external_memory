...
import task.copy
from sam.SAM import Controller
from sam.SAM import SAMCell
import os
import cv2
...
tf.flags.DEFINE_integer("h_size", "100", "hidden dimensions for SAM controller")
...
tf.flags.DEFINE_integer("in_seq_min_len", "2", "input seq max_size")
tf.flags.DEFINE_integer("in_seq_max_len", "4", "input seq max_size")
...
tf.flags.DEFINE_string("save_dir", "sam_checkpoints", "dir for checkpoints")
...
def main(args):
  ...
  ntm = SAMCell(y_size=FLAGS.y_size, batch_size=FLAGS.batch_size, Controller=ctrl, n_mem=FLAGS.N, R=1, W=1, dtype=tf.float32)
  ...
  with tf.Session() as sess:
    ...
    for epoch in xrange(FLAGS.max_epoch):
      ...
      for itr in xrange(FLAGS.max_itrs):
        ...
        x_data, y_data = task.copy.generate_data(FLAGS.x_size, FLAGS.y_size, seq_max, FLAGS.batch_size, FLAGS.in_seq_min_len, FLAGS.in_seq_max_len)
